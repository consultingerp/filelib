...
EXCLUDED_MODELS = r"^_unknown$|e2yun\.workflo.+|res\..+|ir\..+" + \
...
def update_workflow(self):
    ...
    """
    # Variables
    wkf_obj = self.env['e2yun.workflow'].sudo()
...
def inherit_workflow_manager(cr, model):
    ...
    if 'e2yun.workflow.model' in parents:
        parents.remove('e2yun.workflow.model')
...
AND   table_name = 'e2yun_workflow');"""
            cr.execute(sql)
            res = cr.dictfetchall()
            res = res and res[0] or {}
            if res.get('exists', False):
                # Check for model's workflow
                sql = """SELECT * FROM e2yun_workflow wkf, ir_model im 
...
AND   im.model = '%s';""" % model_name
                ...
                for rec in cr.dictfetchall():
                    ...
                    if rec.get('model', False) == model_name:
                        if 'e2yun.workflow.model' not in parents:
                            if hasattr(model, 'state'):
                                delattr(model, 'state')
                            parents.insert(0, 'e2yun.workflow.model')
                        if rec.get('mail_thread_add', False):
                            if 'mail.thread' not in parents:
                                parents.append('mail.thread')
                            if 'mail.activity.mixin' not in parents and rec.get('activities_add', False):
                                parents.append('mail.activity.mixin')

                            if 'portal.mixin' in [x._name for x in model.module_to_models[model._module]] and \
                                            'portal.mixin' not in parents and rec.get('followers_add', False):
