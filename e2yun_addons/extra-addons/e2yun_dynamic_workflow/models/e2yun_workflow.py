...
class E2yunWorkflow(models.Model):
    _name = 'e2yun.workflow'
    _description = 'E2yun Workflow'
    ...
    node_ids = fields.One2many('e2yun.workflow.node', 'workflow_id', string='Nodes')
    ...
    def validate_nodes(self):
        # Objects
        wkf_node_obj = self.env['e2yun.workflow.node']
        for rec in self:
    ...
    def btn_reload_workflow(self):
        from odoo.addons import e2yun_dynamic_workflow
        return e2yun_dynamic_workflow.update_workflow_reload(self)
    ...
    def btn_buttons(self):
        for rec in self:
            act = {
                ...
                'view_mode': 'tree,form',
                'view_id': False,
                'res_model': 'e2yun.workflow.node.button',
                'domain': [('workflow_id', '=', rec.id)],
                'context': {'default_workflow_id': rec.id},
                'type': 'ir.actions.act_window',
            }
...
class E2yunWorkflowNode(models.Model):
    _name = 'e2yun.workflow.node'
    _description = 'E2yun Workflow Nodes'
    _order = 'sequence'
    ...
    out_link_ids = fields.One2many('e2yun.workflow.link', 'node_from', string='Outgoing Transitions')
    in_link_ids = fields.One2many('e2yun.workflow.link', 'node_to', string='Incoming Transitions')
    field_ids = fields.One2many('e2yun.workflow.node.field', 'node_id', string='Fields')
    button_ids = fields.One2many('e2yun.workflow.node.button', 'node_id', string='Buttons')
    workflow_id = fields.Many2one('e2yun.workflow', string='Workflow Ref.', ondelete='cascade', required=True)
...
class E2yunWorkflowLink(models.Model):
    _name = 'e2yun.workflow.link'
    _description = 'E2yun Workflow Links'
    ...
    node_from = fields.Many2one('e2yun.workflow.node', 'Source Node', ondelete='cascade', required=True)
    node_to = fields.Many2one('e2yun.workflow.node', 'Destination Node', ondelete='cascade', required=True)
...
class E2yunWorkflowNodeButton(models.Model):
    _name = 'e2yun.workflow.node.button'
    _description = 'E2yun Workflow Node Buttons'
    _order = 'sequence'
    ...
    link_id = fields.Many2one('e2yun.workflow.link', string='Link')
    ...
    node_id = fields.Many2one('e2yun.workflow.node', string='Workflow Node Ref.', ondelete='cascade', required=True)
    workflow_id = fields.Many2one('e2yun.workflow', string='Workflow Ref.', required=True)
    ...
    def change_workflow(self):
        for rec in self:
            if isinstance(rec.id, int) and rec.node_id and rec.node_id.workflow_id:
                ...
                model_id = self.env['e2yun.workflow.node'].sudo().browse(self.env.context.get('default_node_id')).model_id.id
                rec.workflow_id = self.env['e2yun.workflow'].sudo().search([('model_id', '=', model_id)])
    ...
    def run(self):
        for rec in self:
            ...
            locals_dict = {
                ...
                'date': date,
                'timedelta': timedelta,
                'workflow': self.env['e2yun.workflow'],
...
class E2yunWorkflowNodeField(models.Model):
    _name = 'e2yun.workflow.node.field'
    _description = 'E2yun Workflow Node Fields'
    ...
    node_id = fields.Many2one('e2yun.workflow.node', string='Node Ref.', ondelete='cascade', required=True)
