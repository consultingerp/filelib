...
class Base(models.AbstractModel):
    ...
    def _build_model_attributes(cls, pool):
        ret = super(Base, cls)._build_model_attributes(pool)
    ...
    def create(self, vals):
        ...
        if vals.get('res_model') == 'project.task' and vals.get('res_id'):
            agile_enabled = self.env['project.task'].browse(vals.get('res_id')).agile_enabled
        elif self._name == 'project.task':
            agile_enabled = self.agile_enabled
        if is_module_installed(self.env, 'web_syncer') and agile_enabled:
            self.env.syncer.begin(self)
