...
function load_cart_confirm(){
		...
		$.post('/e2yun_sale_online_shop_extends/get_cart_confirm_info',{
	...
	function confirm(){
		...
		$.post('/e2yun_sale_online_shop_extends/order_confirm',{
			'access_token' : access_token,
			'csrf_token' : access_token,
			'phone':phone,
			'address':address,
			...
			var d = JSON.parse(datas);
				if(!d['success']){
					...
					window.location.href = '/e2yun_sale_online_shop_extends/order_done_page'
				}
	...
	function get_coupon(){
		var access_token = $("input[name='csrf_token']").val();
		$.post('/e2yun_sale_online_shop_extends/get_coupon',{
			'access_token' : access_token,
			'csrf_token' : access_token
		},function(datas){
			var d = JSON.parse(datas);
