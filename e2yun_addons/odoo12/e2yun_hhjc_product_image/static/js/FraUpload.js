...
(function($,undefined){
    $.fn.FraUpload = function(options,param){
            ...
            if(para.fetch=="img"){
                ...
                , onAdd: function (evt){ //拖拽时候添加有新的节点的时候发生该事件
                  ...
                  onEnd: function(evt){ //拖拽完毕之后发生该事件
                    ...
                    for(var k in self.files_all){
                        md5_data.push(self.files_all[k].md5);
                    }
                    $.post('/image/sort',{
                        'product_tmpl_id':getProductId(),
                        'md5_data':JSON.stringify(md5_data)
                    },function(data){})
         ...
         * 返回: 无
         */
        $.fn.FraUpload.view_DOM = function (){
            return view_DOM;
        }
        ...
        $.fn.FraUpload.getsize = function (byte){
            return self.getsize(byte);
        }

        $.fn.FraUpload.loadData = function(img){
            //将文件加入对象
            var objKey = self.objLength(self.files_all);
            self.files_all[objKey] = {
            ...
            var html =  '<li data-md5="'+img.file_md5+'">'+
                                    '<img src=""/>'+
                                        "<div class='file-tit'>"+
                                            "<div title='"+img.file_name+"'>"+img.file_name+"</div>"+
                                            "<div>"+self.getsize(Number(img.file_size))+"</div>"+
                                        "</div>"+
                                    '<div class="file-footer-buttons">'+
                                        '<span class="iconfont icon-delete" title="删除文件"></span>'+
                                    '</div>'+
                                '</li>';
                    view_DOM.append(html);
                    view_DOM.find("li[data-md5='"+img.file_md5+"'] img:eq(0)").attr('src',img.value)
        }
