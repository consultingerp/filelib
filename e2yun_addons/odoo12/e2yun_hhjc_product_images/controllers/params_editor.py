...
class ParamsEditor(http.Controller):
    ...
    def editor_save(self, osv_info, editor_id, delete_items, **args):
        ...
        for index in range(len(args) - len(image_id_keys)):
            ...
            if str(index) + '-id' in args:
                write_id = json.loads(args[str(index) + '-id']).get('id')
                if write_id:
                    write_val = {'index': index}
                else:
                    create_val = {
                        osv_info.get('editor_field', 'product_tmpl_id'): int(editor_id),
        ...
        if delete_items:
            delete_items_ids = list(map(int, json.loads(delete_items)))
    ...
    def images_get(self, osv_info, editor_id, **kws):
        return request.make_response(json.dumps(
            [{
                ...
                'component': 'image-input',
            } for row in request.env['product.image.ext'].search([('product_tmpl_id', '=', int(editor_id))])]
        ))
    ...
    def images_change_order(self, **kws):
        if request.jsonrequest.get('ids') and request.jsonrequest.get('ids') != ['undefined']:
            request.env.cr.execute("""UPDATE product_image_ext ji SET
                                        order_sort=(select order_sort + 100  
                                        from product_image_ext jin where jin.id=ji.id) 
                                         WHERE id in (%s);""" % ','.join(request.jsonrequest.get('ids')))
            for image in request.jsonrequest.get('change_list'):
                if not image.get('id'):
                    continue
                request.env.cr.execute("""UPDATE product_image_ext ji SET  order_sort=%s,is_primary=%s
                                          WHERE id = %s""" % (image.get('index'),
                                                              not int(image.get('index')),
                                                              image.get('id')))
                if not int(image.get('index')):
                    request.env.cr.execute("""UPDATE product_image_ext jp SET image_path='%s' WHERE id ='%s'""" %
                                           (image.get('file_url'), image.get('product_tmpl_id')))
    ...
    def images_add(self, **kws):
        file_url = self._generate_image_path('product.image.ext', kws.get('product_tmpl_id'), kws.get('file'))
        ...
        if kws.get('index') == '0':
            request.env.cr.execute("""UPDATE product_image_ext jp SET image_path='%s' WHERE id = %s""" %
                                   (file_url, kws.get('product_tmpl_id')))
        return request.make_response(json.dumps({'result': {'image_id': image_row.id }, 'code': 'success'}))
