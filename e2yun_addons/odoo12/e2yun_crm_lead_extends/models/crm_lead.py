...
class CrmLead(models.Model):
    ...
    tag_ids = fields.Many2one('crm.lead.tag', string='Opportunity Type', required=True,
    ...
    customer_owner = fields.Many2one('res.users', string='Customer Owner', compute='onchange_partner_id',
    ...
    def onchange_amount_price(self):
        if not self.amount or self.amount == 0:
            ...
            currency_rate = self.env['res.currency']._get_conversion_rate(self.property_product_pricelist.currency_id,
                                                                          company_id.currency_id, company_id,
                                                                          create_date)
            ...
            usd_currency_rate = self.env['res.currency']._get_conversion_rate(
                self.property_product_pricelist.currency_id, usd_currency,
    ...
    def onchange_partner_id(self):
        for line in self:
            line.customer_owner = line.sudo().partner_id.user_id.id
...
class CrmLeadLost(models.TransientModel):
    ...
    lost_reason_id = fields.Many2one('crm.lost.reason', 'Lost Reason', required=True,
                                     default=lambda self: self._default_lost_reason_id())
    losssuspend_detail = fields.Char(string='Loss/Suspend detail', required=True,
                                     default=lambda self: self._default_losssuspend_detail())
