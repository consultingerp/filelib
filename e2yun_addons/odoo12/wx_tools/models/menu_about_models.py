...
class wx_menu(models.Model):
    ...
    def _get_menu_action(self, name, action, entry):
        server_url = entry.server_url

        if action and action._name == 'wx.action.act_url':
            url = server_url + '/web/login?usercode=' + action.code + '&codetype=wx&redirect=' + action.url
            wxoauth = ComponentOAuth(entry.wxclient.appid, component_appid='',
                                     component_access_token=entry.wxclient.token,
                                     redirect_uri=url, scope='snsapi_userinfo', state=action.code)
    ...
    def _get_menu_item(self, name, action, childs, entry):
        if childs:
            child_list = []
            for child in childs:
                child_dict = self._get_menu_action(child.name, child.action, entry)
    ...
    def do_active(self):
        ...
        buttons = []
        if self.left:
            buttons.append(self._get_menu_item(self.left, self.left_action, self.left_ids, entry))
        if self.middle:
            buttons.append(self._get_menu_item(self.middle, self.middle_action, self.middle_ids, entry))
        if self.right:
            buttons.append(self._get_menu_item(self.right, self.right_action, self.right_ids, entry))
