...
class wx_articlesreply_article(models.Model):
    ...
    def get_wx_reply(self):
        return [self.title, self.description, self.get_img_url(), self.get_url()]
    ...
    def get_img_url(self):
        if self.img_type == 'url':
            ...
            return '%s/web/image_user/wx.articlesreply.article/%s/img_file/%s.png/' % (base_url, self.id, self.id)

    def get_url(self):
        entry = client.wxenv(self.env)
        wxclient = entry.wxclient
        server_url = entry.server_url
        url = server_url + '/web/login?usercode=' + self.code + '&codetype=wx&redirect=' + self.url
        wxoauth = ComponentOAuth(entry.wxclient.appid, component_appid='',
                                 component_access_token=entry.wxclient.token,
                                 redirect_uri=url, scope='snsapi_userinfo', state=self.code)
