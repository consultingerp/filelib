...
class RecruitmentSource(models.Model):
    _inherit = 'supplier.recruitment.source'
    ...
    def _compute_url(self):
        ...
        for source in self:
            source.url = urljoin(base_url, "%s?%s" % (source.job_id.website_url,
                url_encode({
                    'utm_campaign': self.env.ref('e2yun_supplier_recruitment.utm_campaign_job').name,
...
class Job(models.Model):
    ...
    _name = 'supplier.job'
    _inherit = ['supplier.job', 'website.seo.metadata', 'website.published.mixin']

    def _get_default_website_description(self):
        default_description = self.env["ir.model.data"].xmlid_to_object("e2yun_website_supplier_recruitment.default_website_description")
    ...
    def _compute_website_url(self):
        ...
        for job in self:
            job.website_url = "/supplierjobs/detail/%s" % job.id
