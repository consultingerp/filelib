...
class RecruitmentSource(models.Model):
    _name = "supplier.recruitment.source"
    ...
    def create_alias(self):
        campaign = self.env.ref('e2yun_supplier_recruitment.utm_campaign_job_supplier')
        ...
        for source in self:
            ...
            source.alias_id = self.with_context(alias_model_name='supplier.applicant', alias_parent_model_name='supplier.job').env['mail.alias'].create(vals)
            source.name = source.source_id.name


class RecruitmentStage(models.Model):
    _name = "supplier.recruitment.stage"
    ...
    job_id = fields.Many2one('supplier.job', string='Job Specific',
                             ondelete='cascade',
    ...
    def default_get(self, fields):
        if self._context and self._context.get('default_job_id') and not self._context.get('supplier_recruitment_stage_mono', False):
...
class RecruitmentDegree(models.Model):
    _name = "supplier.recruitment.degree"
...
class Applicant(models.Model):
    ...
    def _default_stage_id(self):
        if self._context.get('default_job_id'):
            ids = self.env['supplier.recruitment.stage'].search([
                '|',
                ('job_id', '=', False),
    ...
    def _default_company_id(self):
        ...
        if self._context.get('default_product_id'):
            product = self.env['product.template'].browse(self._context['default_product_id'])
            company_id = product.company_id.id
        if not company_id:
            company_id = self.env['res.company']._company_default_get('supplier.applicant')
        return company_id
    ...
    stage_id = fields.Many2one('supplier.recruitment.stage', 'Stage', track_visibility='onchange',
    ...
    categ_ids = fields.Many2many('supplier.applicant.category', string="Tags")
    ...
    job_id = fields.Many2one('supplier.job', "Applied Job")
    ...
    supplier_id = fields.Many2one('product.supplierinfo', string="Supplier", track_visibility="onchange", help="Supplier linked to the applicant.")
    user_email = fields.Char(related='user_id.email', type="char", string="User Email", readonly=True)
    attachment_number = fields.Integer(compute='_get_attachment_number', string="Number of Attachments")
    supplier_name = fields.Many2one(related='supplier_id.name', string="Supplier Name")
    attachment_ids = fields.One2many('ir.attachment', 'res_id', domain=[('res_model', '=', 'upplier.applicant')], string='Attachments')
    ...
    min_qty_extra = fields.Char("Min Qty Extra", help="Min Qty by Applicant, extra advantages")
    price = fields.Float(
        'Price', default=0.0, digits=dp.get_precision('Product Price'),
        required=True, help="The price to purchase a product")
    price_extra = fields.Char("Price Extra", help="Price by Applicant, extra advantages")
    delay = fields.Integer(
        'Delivery Lead Time', default=1, required=True,
        help="Lead time in days between the confirmation of the purchase order and the receipt of the products in your warehouse. Used by the scheduler for automatic computation of the purchase order planning.")
    delay_extra = fields.Char("Delay Extra", help="Delay by Applicant, extra advantages")
    ...
    def _get_attachment_number(self):
        read_group_res = self.env['ir.attachment'].read_group(
            [('res_model', '=', 'supplier.applicant'), ('res_id', 'in', self.ids)],
            ['res_id'], ['res_id'])
    ...
    def onchange_job_id(self):
        ...
        self.product_id = vals['value']['product_id']
    ...
    def _onchange_job_id_internal(self, job_id):
        ...
        if job_id:
            job = self.env['supplier.job'].browse(job_id)
            product_id = job.product_id.id
            user_id = job.user_id.id
            if not self.stage_id:
                stage_ids = self.env['supplier.recruitment.stage'].search([
                    '|',
                    ('job_id', '=', False),
                    ('job_id', '=', job.id),
                    ('fold', '=', False)
    ...
    def _onchange_stage_id_internal(self, stage_id):
        if not stage_id:
            return {'value': {}}
        stage = self.env['supplier.recruitment.stage'].browse(stage_id)
        if stage.fold:
    ...
    def create(self, vals):
        if vals.get('product_id') and not self._context.get('default_product_id'):
            self = self.with_context(default_product_id=vals.get('product_id'))
    ...
    def get_empty_list_help(self, help):
        return super(Applicant, self.with_context(empty_list_help_model='supplier.job',
    ...
    @api.multi
    def action_get_created_supplier(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window'].for_xml_id('e2yun_supplier_recruitment', 'open_view_supplier_list')
        action['res_id'] = self.mapped('supplier_id').ids[0]
        return action
    ...
    def action_makeMeeting(self):
        ...
        """
        self.ensure_one()
        partners = self.partner_id | self.user_id.partner_id | self.product_id.manager_id.user_id.partner_id

        category = self.env.ref('e2yun_supplier_recruitment.categ_meet_interview_supplier')
    ...
    def action_get_attachment_tree_view(self):
        ...
        action['search_view_id'] = (self.env.ref('e2yun_supplier_recruitment.ir_attachment_view_search_inherit_supplier_recruitment').id, )
        return action
    ...
    def _track_subtype(self, init_values):
        record = self[0]
        if 'supplier_id' in init_values and record.supplier_id:
            return 'e2yun_supplier_recruitment.mt_applicant_certified_supplier'
        elif 'stage_id' in init_values and record.stage_id and record.stage_id.sequence <= 1:
            return 'e2yun_supplier_recruitment.mt_applicant_new_supplier'
        elif 'stage_id' in init_values and record.stage_id and record.stage_id.sequence > 1:
            return 'e2yun_supplier_recruitment.mt_applicant_stage_changed_supplier'
    ...
    def message_get_reply_to(self, ids, default=None):
        ...
        aliases = self.env['supplier.job'].message_get_reply_to(applicants.mapped('job_id').ids, default=default)
    ...
    @api.multi
    def create_supplier_from_applicant(self):
        """ Create an hr.supplier from the supplier.applicants """
        supplier = False
        for applicant in self:
            ...
            if applicant.job_id and (applicant.partner_name or contact_name):
                applicant.job_id.write({'no_of_certified_supplier': applicant.job_id.no_of_certified_supplier + 1})
                if applicant.partner_id:
                    supplier = self.env['product.supplierinfo'].create({'name': applicant.partner_id.id,
                                                   'job_id': applicant.job_id.id,
                                                   'product_tmpl_id': applicant.product_id.id or False,
                                                    'min_qty': applicant.min_qty or 0,
                                                    'price': applicant.price or 0.0,
                                                    'delay': applicant.delay or  0})
                else:
                    partner= self.env['res.partner'].create({'name': applicant.partner_name,
                                                             'supplier': True,
                                                             'email': applicant.email_from or False,
                                                             'phone': applicant.partner_phone or False,
                                                             'mobile': applicant.partner_mobile or False})
                    supplier = self.env['product.supplierinfo'].create({'name': partner.id,
                                                    'job_id': applicant.job_id.id,
                                                    'product_tmpl_id': applicant.product_id.id or False,
                                                    'min_qty': applicant.min_qty or 0,
                                                    'price': applicant.price or 0.0,
                                                    'delay': applicant.delay or  0})
                applicant.write({'supplier_id': supplier.id})
                applicant.job_id.message_post(
                    body=_('New Supplier %s Certified') % applicant.partner_name if applicant.partner_name else applicant.name,
                    subtype="mt_job_applicant_certified_supplier")
                supplier._broadcast_welcome()
            else:
                raise UserError(_('You must define an Applied Job and a Contact Name for this applicant.'))

        supplier_action = self.env.ref('e2yun_supplier_recruitment.open_view_supplier_list')
        dict_act_window = supplier_action.read([])[0]
        if supplier:
            dict_act_window['res_id'] = supplier.id
...
class ApplicantCategory(models.Model):
    _name = "supplier.applicant.category"
